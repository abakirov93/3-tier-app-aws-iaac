...
resource "aws_instance" "3-tier-beksalizhan" {
  ...
  tags {
    BuiltWith = "terraform"
    Name      = "3-tier-beksalizhan"
  }

  vpc_security_group_ids = ["${ aws_security_group.3-tier-beksalizhan-security-group.id }"] # attaching security group
